...
//송금 기능 송금 내역 조회 GET 클라이언트 2 입장
const GetTransferAllCopy = () => {
    ...
    const getSelTransferAllList = useCallback(async (queryString) => {
        setLoading(true);
        try {
            const resp = await (await axios.get(`//localhost:8080/transfer/alls?${queryString}`)).data;
...
export default GetTransferAllCopy;