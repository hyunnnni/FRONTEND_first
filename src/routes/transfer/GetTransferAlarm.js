...
//송금 기능 송금 알람 조회 후 수락/거절 GET/PUT 클라이언트 1 입장
const GetTransferAlarm = () => {
    ...
    const getTransferAlarmList = useCallback(async (queryString) => {
        setLoading(true);
        try {
            const resp = await (await axios.get(`//localhost:8080/transfer/alarm?${queryString}`)).data;
...
export default GetTransferAlarm;