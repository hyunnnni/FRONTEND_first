...
//계약 송수신 기능 거래 알람 조회 후 거절/수락 GET/PUT 클라이언트 1입장
  const RemitGetAlarm = () => {
    ...
    const getAlarmList = useCallback(async (queryString) => {
        setLoading(true);
        try {
            const resp = await (await axios.get(`//localhost:8080/transaction/select?${queryString}`)).data;
            setAlarmList(resp);
            setLoading(false);
            console.log(resp);
        } catch (err) {
    ...
    const handleConfirm = async() => {
        ...
        try {
          const res = await axios.put(`//localhost:8080/transaction/resp`, postData);
          if(res.data === 1){
            alert('수락하였습니다.');
          }else{
            alert('수락을 실패했습니다.');
